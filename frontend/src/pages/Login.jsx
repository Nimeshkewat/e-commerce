...
function Login() {
  ...
  return (
    ...
    <Link to='/'><h2 className='fixed top-5 left-5 hover:text-gray-300 text-white transition-all duration-500 cursor-pointer font-bold text-lg  md:text-2xl lg:text-3xl '>N-MART</h2></Link>
      <form onSubmit={handleSubmit} className='border h-screen bg-black flex flex-col items-center transition-all duration-500'>
        <div className='mt-40 flex flex-col w-1/2 md:w-1/4 gap-7'>
          ...
          <div className='flex flex-col gap-7'>
            <div className='flex '>
              ...
              <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" id="email" className='focus:bg-white focus:text-black py-3 px-5 w-full rounded-lg text-white font-bold border-none outline-1' placeholder='Enter email address' />

            </div>
            <div className='flex'>
              <label htmlFor="password"></label>
              <input value={password} onChange={(e)=>setPassword(e.target.value)} type="password" id="password" className='focus:bg-white focus:text-black py-3 px-5 w-full rounded-lg text-white font-bold border-none outline-1' placeholder='Enter your password' />
            </div>
          </div>
